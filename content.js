...
// === Collection Mode (for sidepanel tweet analyzer) ===
...
window.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'XVM_START_COLLECT') {
    collectMode = true;
    collectStore.clear();
  ...
  if (event.data?.type === 'XVM_STOP_COLLECT') {
    collectMode = false;
  }
});

// When in collect mode, also push newly scanned tweets to the bridge
const originalScanForTweets = scanForTweets;
// Patch scanForTweets to also report to bridge
const _origSetOnStore = tweetDataStore.set.bind(tweetDataStore);
tweetDataStore.set = function(id, data) {
  _origSetOnStore(id, data);
  if (collectMode && !collectStore.has(id)) {
    collectStore.set(id, data);
    // Batch: send collected tweets to bridge every time
    window.postMessage({
      type: 'XVM_COLLECT_DATA',
      tweets: [data],
    }, '*');
  }
  return this;
...
// Auto-scroll for collection mode
function startAutoScroll() {
  ...
  const scrollInterval = setInterval(() => {
    ...
    // No new content for 6 seconds → done
    if (stableCount >= 4) {
      clearInterval(scrollInterval);
      collectMode = false;
      window.postMessage({ type: 'XVM_SCROLL_DONE' }, '*');
    }
  }, 1500);
}

// Detect when search page is loaded and start scrolling
