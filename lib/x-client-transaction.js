...
(() => {
  ...
  async function build() {
    ...
    // Resolve chunk id → hash for "ondemand.s". Anchored to webpack's chunk
    // loader runtime (`}[e] || e) + "." + {`) — more precise than naive
    // `<id>:"<hex>"` which can match unrelated mappings on the page.
    // Adapted from Lqm1/x-client-transaction-id PR #18 (2026-04).
    const RUNTIME_HASH_RE = /(\d+):\s*["']ondemand\.s["'][\s\S]*?\}\)?\[e\]\s*\|\|\s*e\)\s*\+\s*["']\.["']\s*\+\s*\(?\{[\s\S]*?\b(\d+):\s*["']([a-f0-9]{6,12})["']/;
    const runtimeMatch = homeHtml.match(RUNTIME_HASH_RE);
    let chunkId = null;
    let hash = null;
    if (runtimeMatch && runtimeMatch[1] === runtimeMatch[2]) {
      chunkId = runtimeMatch[1];
      hash = runtimeMatch[3];
    } else {
      // Fallback: locate chunk id via name, then take the LAST hash mapping
      // for that id (the page contains both `<id>:"name"` and `<id>:"hash"`
      // pairs, and the hash table comes after the name table).
